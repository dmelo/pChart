   ...
   function draw($X,$Y,$Width,$Height,$Format="")
    {
     ...
     if ( !is_array($Values) ) { $Value = $Values; $Values = array(); $Values[] = $Value; }
     ...
     foreach ($IndicatorSections as $Key => $Settings)
      {
       ...
       if ( $Key == 0 && $DrawLeftHead )
        {
         $Poly = array(); $Poly[] = $X1-1; $Poly[] = $Y; $Poly[] = $X1-1; $Poly[] = $Y+$Height; $Poly[] = $X1-1-$HeadSize; $Poly[] = $Y+($Height/2);
       ...
       if ( $ValueDisplay == INDICATOR_VALUE_LABEL )
        {
         if ( empty($Break) )
          ...
          {
           sort($Break);
           $Poly = array(); $Poly[] = $X1; $Poly[] = $Y; $LastPointWritten = FALSE;
           foreach($Break as $iKey => $Value)
            {
             ...
             elseif ($X1 - ($Value-5) > 0 )
              {
               $Offset = $X1 - ($Value-5);
               $Poly = array(); $Poly[] = $X1; $Poly[] = $Y + $Offset;
              }
       ...
       if ( $Key == count($IndicatorSections)-1 && $DrawRightHead )
        {
         $Poly = array(); $Poly[] = $X2+1; $Poly[] = $Y; $Poly[] = $X2+1; $Poly[] = $Y+$Height; $Poly[] = $X2+1+$HeadSize; $Poly[] = $Y+($Height/2);
         $this->pChartObject->drawPolygon($Poly,$Color);
